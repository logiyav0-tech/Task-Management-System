...
const TaskTable = ({ tasks, onTaskUpdate, allowEditCompleted, onTaskDelete }) => {
    ...
    const exportToCSV = useCallback(async () => {
        ...
        try {
            ...
            link.setAttribute('download', `ZOHO-tasks-${new Date().toISOString().split('T')[0]}.csv`);
    ...
    // Generate Report function - Opens Outlook with professional email template
    const generateReport = useCallback(() => {
        if (tasks.length === 0) {
            alert('No tasks to generate report!');
            return;
        }
        ...
        const subject = `ZOHO Task Management Report - ${new Date().toISOString().split('T')[0]}`;
        
        const body = `
Dear Team,

Please find below the Task Management System report for ZOHO:
...
KEY HIGHLIGHTS:
${completedCount > 0 ? `✓ ${completedCount} tasks successfully completed` : 'No tasks completed yet'}
${criticalCount > 0 ? `⚠ ${criticalCount} critical tasks require immediate attention` : 'No critical tasks'}
${overdueTasks > 0 ? `🚨 ${overdueTasks} tasks are overdue` : 'No overdue tasks'}

PRIORITY ACTIONS:
${criticalCount > 0 ? '• Please review critical tasks and allocate resources accordingly' : '• All tasks are within normal priority range'}
${overdueTasks > 0 ? '• Immediate action required for overdue tasks' : '• All tasks are on schedule'}

NEXT STEPS:
1. Review attached detailed task report
2. Address critical and overdue tasks promptly
3. Update task statuses regularly
4. Coordinate with responsible team members

This report has been automatically generated by the ZOHO Task Management System.
...
---
Confidentiality Notice: This report contains proprietary information intended only for authorized personnel.
        `.trim();
        ...
        // Also provide option to download as text file
        const reportContent = `TASK MANAGEMENT REPORT\n${'='.repeat(50)}\n${body}`;
        const blob = new Blob([reportContent], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `task-report-${new Date().toISOString().split('T')[0]}.txt`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        alert('Report generated! Outlook email opened and text file downloaded.');
    }, [tasks, completedCount, criticalCount, inProgressCount, notStartedCount]);
        ...
        <div className="professional-table-container">
            ...
            <div className="table-summary">
                ...
                <div className="summary-actions">
                    ...
                    <button 
                        className="summary-btn generate-report-btn" 
